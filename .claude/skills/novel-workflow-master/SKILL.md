...
用户在创意入口输入「测试模式」「测试」或「test」时触发。测试模式固定参数（中篇，30 章，单章目标 1500-2200 字），从阶段二开始连续自动推进到阶段七，禁止中途询问是否继续。仅当章节蓝图/正文/编辑报告/发布包全部达标时才可宣告完成。默认输出：完整主线 + 30 章章节蓝图 + 30 章正文 + 一致性报告 + 发布包。
