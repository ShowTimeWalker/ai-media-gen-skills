...
description: Coordinate video generation and default delivery without binding to a specific AI or storage provider.
...
当用户表达视频生成需求时，默认完成态是“返回可访问视频结果”：

1. 由多媒体内容生成 Agent 选择视频生成供应商
2. 生成视频并保存到本地
3. 默认进入对象存储分发步骤
4. 返回可访问链接
...
- 具体生成由 provider skill 负责，例如 `juzi_image_video_generation`
