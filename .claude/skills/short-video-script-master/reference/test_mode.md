...
| 阶段三：剧本 | 剧本+片段拆分一步到位，按篇章推进 | 自动生成第一篇章剧本 + 第一集完整片段拆分 |
...
**2. 场景参考图 Prompt（≤5个场景）**
- 从 framework.md 主要场景中选取最重要的 ≤5 个场景
- 每场景生成1张参考图 prompt（无人物）

**3. 关键帧 Prompt（第一集全部片段）**
- 为第一集所有片段生成首帧和末帧 prompt
...
- `screenplays/Chapter1_*/E01.md` — 第一集完整剧本 + 片段拆分
...
- 固定参数（720p、1min×30集）覆盖用户在阶段一的选择
- 角色数量上限（≤5）覆盖 framework.md 中定义的全部角色
- 场景数量上限（≤5）覆盖 framework.md 中定义的全部场景
