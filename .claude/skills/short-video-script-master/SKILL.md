...
通过三步引导确认基础参数（画风 → 规格 → 类型），再通过自由对话深化创意素材。查阅 `reference/step1_ideation.md` 获取完整的对话引导策略。当用户提到具体类型时，查阅 `reference/hongguo_genre_map.md` 向用户展示类型特征和爆款公式。当画风、规格、类型、核心情绪、一句话灵感五个维度明确后，进入阶段二。

### 阶段二：核心框架定调

基于阶段一已确认的画风和规格，生成整部剧的核心框架，包括：主要剧情概述、核心人物（年龄、具体形象）、主要场景、画风细化（色调/质感/比例）、整剧基调。查阅 `reference/step2_core_framework.md` 获取完整的输出格式和关键规则。**必须等待用户二次确认**才能进入阶段三。
...
按篇章逐步推进。查阅 `reference/step4_screenplay.md` 获取完整规范。

**4A — 剧本生成**：读取篇章故事文件，为该篇章所有集数生成剧本（场景列表、画面、旁白对白、声音设计、结尾钩子）。用户确认后进入 4B。

**4B — 片段拆分**：将每集剧本拆分为 6-8 秒的 AI 视频片段，用朴实平白的语言描述画面内容（禁止比喻、华丽辞藻、抽象概念）。用户确认后展示**篇章完成选项**：
