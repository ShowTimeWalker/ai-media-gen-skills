...
通过定时任务实现轮询，不使用脚本内部 sleep 循环。每次轮询是一个独立的一次性定时任务，触发后查询状态，根据结果决定下一步。
...
1. 同一内容类型内，按优先级从高到低尝试
2. 当前供应商脚本报错（非零退出码）或任务失败，尝试下一个优先级供应商
3. 用户指定了供应商时不降级，直接报告该供应商的失败结果
4. 所有供应商都失败，汇总各供应商的错误信息告知用户
