...
def _encode_form_fields(fields: list[tuple[str, str]]) -> tuple[bytes, str]:
    boundary = "----CodexJuziSkillFormBoundary7MA4YWxkTrZu0gW"
    chunks: list[bytes] = []
...
def query_video_task(juzi_id: str) -> dict[str, Any]:
    return ensure_success(get_json("/open/api/vNotify", {"juzi_id": juzi_id}), "视频进度查询")
...
def create_image_task(
    *,
    ...
    fields: list[tuple[str, str]] = [("model", model), ("prompt", prompt)]
    if ratio:
...
def query_image_task(juzi_id: str) -> dict[str, Any]:
    return ensure_success(get_json("/open/api/imgNotify", {"juzi_id": juzi_id}), "图片进度查询")


def wait_for_video_task(juzi_id: str, *, poll_interval: int = 15, timeout: int = 900) -> dict[str, Any]:
...
def wait_for_image_task(juzi_id: str, *, poll_interval: int = 5, timeout: int = 120) -> dict[str, Any]:
    deadline = time.monotonic() + timeout
    last_status: str | None = None
...
def extract_video_result_url(response: dict[str, Any]) -> str:
    result_url = str(response.get("data", {}).get("juzi_url", "")).strip()
    if not result_url:
...
def extract_image_result_url(response: dict[str, Any]) -> str:
    sai_url = response.get("data", {}).get("sai_url")
