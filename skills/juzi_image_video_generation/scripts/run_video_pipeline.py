...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--image-url", action="append", dest="image_urls", help="参考图 URL，可传多次")
    parser.add_argument("--no-reference-images", action="store_true", help="不使用默认参考图")
    parser.add_argument("--poll-interval", type=int, default=DEFAULT_POLL_INTERVAL, help="轮询间隔秒数，默认 15")
    parser.add_argument("--timeout", type=int, default=DEFAULT_TIMEOUT, help="超时秒数，默认 900")
    parser.add_argument("--expires-in", type=int, default=DEFAULT_EXPIRES_IN, help="分享链接有效期秒数，默认 600")
    parser.add_argument("--object-prefix", default=DEFAULT_OBJECT_PREFIX, help="七牛对象 key 前缀")
