...
DEFAULT_PROMPT = "人物保持清晰，自然眨眼并轻微转头，镜头稳定推进。"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="海螺首帧图生视频测试脚本")
    parser.add_argument(
        "--first-frame-image",
    ...
    parser.add_argument("--output", type=Path, help="输出文件路径，默认写入 outputs/hailuo/image_to_video/")
...
def main() -> None:
    ...
    output_path = args.output or default_output_path("image_to_video")

    print(f"使用模型: {args.model}")
    print(f"请求参数: {json.dumps({**payload, 'first_frame_image': '<image>'}, ensure_ascii=False)}")
